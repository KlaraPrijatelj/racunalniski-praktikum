\begin{frame}{Konstrukcija pravokotnice na premico $p$ skozi točko $T$}
	...
	\begin{itemize}
	  \frametitle{Konstrukcija pravokotnice na premico $p$ skozi točko $T$}
		...
		\item \only<7>{Premica skozi točki $T$ in $C$ je pravokotna na $p$.}
		\column{0.45\textwidth}
		\begin{tikzpicture}
		\centering
			\only<1>{\includegraphics[width=50mm]{fig-1.png}}
			\only<2>{\includegraphics[width=50mm]{fig-2.png}}
			\only<3>{\includegraphics[width=50mm]{fig-3.png}}
			\only<4>{\includegraphics[width=50mm]{fig-4.png}}
			\only<5>{\includegraphics[width=50mm]{fig-5.png}}
			\only<6>{\includegraphics[width=50mm]{fig-6.png}}
			\only<7>{\includegraphics[width=50mm]{fig-7.png}}
		\end{tikzpicture}
		\end{columns}
	  ...
	  \frametitle{Konstrukcija pravokotnice na premico $p$ skozi točko $T$}
		...
		\centering
			 % Sliko smo naredili tako, da so točke A, B, T in C vse enako oddaljene
				 ...
				 \draw (B) circle (1.5pt);
				 \pause
				 % Definicija pomožne točke T' in risanje krožnega loka m, ki se začne v T'
				 \coordinate (T') at ({-\td-\td},{-\tt});
				 \draw[gray, thin] (T') arc[start angle=300, end angle=410, radius=2] node[left] {\scriptsize $m$};
				 \pause
				 % Definicija pomožne točke B' in risanje krožnega loka n, ki se začne v B'
				 \coordinate (B') at ({\td+\td},{\tt});
				 \draw[gray, thin] (B') arc[start angle=135, end angle=240, radius=2] node[right] {\scriptsize $n$};
				 \pause
				 % Točka C
				 \coordinate [label= right:{\scriptsize $C$}] (C) at (0,-\tt+\td);
				 \draw (C) circle (1.5pt);
				 \pause
		\end{tikzpicture}
		\end{columns}
